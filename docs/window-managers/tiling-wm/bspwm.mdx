...
<TabItem value="wm_1" label="Default" default>
...
<TabItem value="wm_2" label="Kitty" default>

**kitty** is the secondary/alternate terminal. In the Bspwm window manager, kitty is launched with an alternative config file. If you need to change anything, make sure you modify the config files in `~/.config/bspwm/kitty` directory. These are the following config files you can modify to change the behaviour of your terminal:

**`~/.config/bspwm/kitty/kitty.conf`** : Main configuration file <br/>
**`~/.config/bspwm/kitty/colors.conf`** : Colors configurations <br/>
**`~/.config/bspwm/kitty/fonts.conf`** : Fonts configurations
...
When you change the `theme`, the **fonts.toml**, **colors.toml** file of alacritty, **colors.conf** file of kitty get overwritten by the theme script.
...
**hsetroot** sets the wallpaper in the Bspwm. If you want to change the wallpaper, Edit `~/.config/bspwm/themes/wallpaper.sh` script and add the path to your wallpaper.
```bash
