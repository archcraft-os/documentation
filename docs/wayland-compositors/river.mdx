...
[![video](https://raw.githubusercontent.com/archcraft-os/misc-files/main/screenshots/river/main.gif)](https://youtu.be/MwnK6arB2Rc)
...
- Download **river_YYYY-MM-DD.tar.gz** file from [ko-fi](https://ko-fi.com/s/69957c0587), and save it in a directory.
...
- Install the following programs on your Linux distro: `river` `wlr-randr` `swaybg` `swayidle` `swaylock` `hyprpicker` `hyprlock` `wl-clipboard` `waybar` `wofi` `foot` `mako` `grim` `slurp` `wf-recorder` `light` `yad` `thunar` `geany` `mpv` `mpd` `mpc` `viewnior` `imagemagick` `xfce-polkit` `xorg-xwayland` `xdg-desktop-portal-wlr` `playerctl` `pastel` `python-pywal` `alacritty` `rofi` `pulsemixer`
...
**`~/.config/river/kitty/kitty.conf`** : Main configuration file <br/>
**`~/.config/river/kitty/colors.conf`** : Colors configurations <br/>
...
**`00-vars.sh`** : Variables and helpers <br/>
**`10-startup.sh`** : Startup programs <br/>
**`20-output.sh`** : Output/monitors config <br/>
**`30-input.sh`** : Input devices config <br/>
**`40-theme.sh`** : River theme config <br/>
**`50-rules.sh`** : Window rules config <br/>
**`60-modes.sh`** : Modes config <br/>
**`70-keybindings.sh`** : Keybindings config <br/>
**`80-tags.sh`** : Tags config <br/>
**`90-mouse.sh`** : Mosue binding config  <br/>
...
Light theme for River.

![img](https://raw.githubusercontent.com/archcraft-os/misc-files/main/screenshots/river/main_light.png)
...
![gif](https://raw.githubusercontent.com/archcraft-os/misc-files/main/screenshots/river/lock.gif)
...
To change GTK theme, icons and cursor in River session, Edit `~/.config/river/scripts/gtkthemes` file and restart River to apply the changes.
```bash
THEME='Arc-Dark'
ICONS='Arc-Circle'
FONT='Noto Sans 9'
...
2. Edit `~/.config/river/config.d/20-output.sh` file and use the information you get by the command above:
```bash
## Output eDP-1 : Laptop Display
wlr-randr --output "eDP-1" \
  --preferred \
  --mode 1920x1080@60.027000 \
  --pos 0,0 \
  --transform normal \
  --scale 1.0 \
