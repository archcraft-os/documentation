...
- Download **newm_YYYY-MM-DD.tar.gz** file from [ko-fi](https://ko-fi.com/s/524182856f), and save it in a directory.
...
- Install the following programs on your Linux distro: [`newm-next-git`](https://github.com/newm-next/newm-next), `wtype` `alacritty` `wl-clipboard` `waybar` `wofi` `foot` `mako` `grim` `slurp` `wf-recorder` `light` `yad` `wlogout` `thunar` `geany` `mpv` `mpd` `mpc` `viewnior` `imagemagick` `xfce-polkit` `xorg-xwayland` `xdg-desktop-portal` `xdg-desktop-portal-wlr` `xdg-desktop-portal-gtk` `playerctl`
- After installing programs above, Create _newm_ directory in **`~/.config`**
```bash
$ mkdir -p ~/.config/newm
```
- Copy Everything from _`dotfiles`_ to **`~/.config/newm`**
```bash
$ cp -r ./dotfiles/* ~/.config/newm/
```
...
**kitty** is the secondary/alternate terminal. In the NEWM wayland compositor, kitty is launched with an alternative config file. If you need to change anything, make sure you modify the config files in `~/.config/newm/kitty` directory. These are the following config files you can modify to change the behaviour of your kitty terminal:

**`~/.config/newm/kitty/kitty.conf`** : Main configuration file <br/>
**`~/.config/newm/kitty/colors.conf`** : Colors configurations <br/>
**`~/.config/newm/kitty/fonts.conf`** : Fonts configurations
...
To toggle blur mode, click on module with `toggle icon` on waybar.
...
To change GTK theme, icons and cursor in NEWM session, Edit `~/.config/newm/config.py` script and restart NEWM to apply the changes.
```py
theme = {
    "gtk_theme": "Nordic",
