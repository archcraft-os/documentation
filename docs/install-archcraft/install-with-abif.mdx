...
> Note: Choose `/boot` if you want **systemd-boot** as your bootloader.
>
> **grub** works with both `/boot` and `/boot/efi`. But preferably, use `/boot/efi`
...
Select `grub` (or `systemd-boot` if you want) as bootloader and press <kbd>Enter</kbd>
